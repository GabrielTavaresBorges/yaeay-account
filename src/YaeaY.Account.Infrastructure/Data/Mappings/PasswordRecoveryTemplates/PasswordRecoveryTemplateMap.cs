...
public sealed class PasswordRecoveryTemplateMap : IEntityTypeConfiguration<PasswordRecoveryTemplate>
{
    public void Configure(EntityTypeBuilder<PasswordRecoveryTemplate> builder)
    {
        ...
        {
            email.Property(value => value.EmailAddress)
                .HasColumnName("FromEmail")
                .HasMaxLength(254)
                .IsRequired();
        });

        builder.Navigation(item => item.FromEmail)
            .IsRequired();

        builder.Property(item => item.FromName)
            .HasMaxLength(150)
            .IsRequired();
        ...
        builder.Property(item => item.BodyHtml)
            .HasColumnType("text")
            .IsRequired();
        ...
        builder.HasIndex(item => new { item.Purpose, item.IsActive })
            .HasDatabaseName("UX_PasswordRecoveryTemplates_Active_Purpose")
            .IsUnique()
